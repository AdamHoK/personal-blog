...
from models import Base, Link, User, Post
...
def blog():
  if request.method == 'POST':
    ...
    if not author:
      return f'<div class="error">Author is required</div>'
    elif not title:
      return f'<div class="error">Title is required</div>'
    elif not content:
      return f'<div class="error">Content is required</div>'
    with Session(engine) as session:
          ...
          blogPost = Post(
            ...
            updated_at=current_time_in_seconds,
            title=title,
            content=content,
            author=author,
            active=True
          )
          session.add_all([blogPost])
          session.commit()
          return f'<div class="success">Blog post created. Delete ID: ({delete_id}) </div><p class="p-2 text-center"> Your blog post is now available at <a href={request.base_url}/p/{blogPost.title} target="_blank">{request.base_url}u/{blogPost.title}</a> </p>'


@app.route("/blog/p/<title>", methods=(["GET", "POST"]))
def get_post(title):
    title = str(title)
    ...
    if request.method == 'POST':
      ...
      post_to_delete = session.scalars(select(Post).where(Post.delete_id.in_([delete_id]))).first()
      session.delete(post_to_delete)
      session.commit()
      return f'<div class="error">Post <strong>{post}</strong> info deleted! <p><a href="{url_for("home")}">Go back home</a></p></div>'

    post = session.scalars(select(Post).where(Post.title.like(title))).first()
    
    if post is not None:
      boolVal = str(post.title.lower() == title)
      return render_template("blogpost.html", post=post, page_url=url_for('get_post', title=title))
    else:
    ...
    if request.method == 'POST':
      ...
      if not user_name:
        ...
        with Session(engine) as session:
          ...
          session.commit()
          return f'<div class="success">Account added. Delete ID: ({delete_id}) </div><p class="p-2 text-center"> Your social links are now available at <a href={request.base_url}u/{user.user_name} target="_blank">{request.base_url}u/{user.user_name}</a> </p>'
